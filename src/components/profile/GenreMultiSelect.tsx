...
export function GenreMultiSelect({ values, onChange }: { values: string[]; onChange: (values: string[]) => void }) {
  ...
  return <fieldset className="sm:col-span-2">
    ...
    <div className="flex flex-wrap gap-2">{values.map((genre) => <button key={genre} type="button" onClick={() => onChange(values.filter((item) => item !== genre))} aria-label={`Remover ${genre}`} className="min-h-10 rounded-full border border-danger bg-danger/20 px-3 text-xs font-black text-danger">{genre} ×</button>)}</div>
