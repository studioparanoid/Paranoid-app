...
export function GenreMultiSelect({ values, onChange }: { values: string[]; onChange: (values: string[]) => void }) {
  ...
  return <fieldset className="sm:col-span-2">
    ...
    <div className="relative mt-2">
      ...
      {open && options.length > 0 && <div id={`${id}-options`} role="listbox" className="absolute inset-x-0 top-[calc(100%+0.25rem)] z-30 max-h-56 overflow-y-auto rounded border border-[var(--border)] bg-[var(--dropdown-background)] p-1 shadow-xl">{options.map((option) => <button key={option} type="button" role="option" aria-selected="false" onMouseDown={(event) => event.preventDefault()} onClick={() => add(option)} className="block min-h-11 w-full rounded px-3 text-left text-sm font-bold text-zinc-200 hover:bg-zinc-900">{option}</button>)}</div>}
    </div>
  </fieldset>;
}
