...
function Section({ title, children }: { title: string; children: ReactNode }) {
  return <section className="border-t border-[var(--border)] pt-6"><h2 className="text-xs font-black uppercase text-danger">{title}</h2><div className="mt-4">{children}</div></section>;
}
...
export function StructuredEventSections({ eventId }: { eventId: string }) {
  ...
  return <div className="space-y-7">
    ...
    {data.tickets.length > 0 && <Section title="Bilhetes"><ul className="divide-y divide-[var(--border)]">{data.tickets.map((ticket) => <li key={ticket.id} className="flex flex-wrap items-center justify-between gap-4 py-3"><div><strong>{ticket.name}</strong><p className="text-xs text-[var(--foreground-muted)]">{[ticket.channelName, ticket.availableCapacity != null ? `${ticket.availableCapacity} disponíveis` : null, ticket.description].filter(Boolean).join(" · ")}</p></div><div className="flex items-center gap-3"><span className="font-black">{ticket.priceAmount == null ? "Consultar" : `${ticket.priceAmount.toFixed(2)} ${ticket.currency}`}</span>{ticket.purchaseUrl && <a href={ticket.purchaseUrl} target="_blank" rel="noreferrer" className="text-xs font-black text-danger hover:text-danger">Comprar</a>}</div></li>)}</ul></Section>}
