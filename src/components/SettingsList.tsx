...
export function SettingsList({ items }: { items: SettingsListItem[] }) {
  return <div className="divide-y divide-[var(--border)] border-y border-[var(--border)]">
    {items.map((item) => {
      const content = <>
        <span className={`grid h-9 w-9 shrink-0 place-items-center rounded bg-[var(--surface-secondary)] ${item.tone === "danger" ? "text-red-500" : "text-[var(--foreground-muted)]"}`}><AppIcon name={item.icon} className="h-4 w-4" /></span>
        <span className="min-w-0 flex-1"><span className={`block text-sm font-bold ${item.tone === "danger" ? "text-red-400" : "text-[var(--foreground)]"}`}>{item.label}</span>{item.description && <span className="mt-0.5 block text-xs text-[var(--foreground-muted)]">{item.description}</span>}</span>
