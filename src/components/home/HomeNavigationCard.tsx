...
export function HomeNavigationCard({ href, image, imageAlt, eyebrow, title, description, eager = false }: HomeNavigationCardProps) {
  return (
    <article className="group interactive-card relative aspect-square min-w-0 overflow-hidden rounded-lg bg-background sm:aspect-[4/3]">
      <Image
        src={image}
        alt={imageAlt}
        fill
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 z-20 flex items-end justify-between gap-4 p-6 sm:p-8">
        <div>
          <p className="text-[0.65rem] font-black uppercase tracking-[0.32em] text-danger">{eyebrow}</p>
          <h2 className="mt-2 text-4xl font-black leading-none sm:text-5xl">{title}</h2>
          <p className="mt-3 text-sm font-bold text-foreground-secondary sm:text-base">{description}</p>
        </div>
