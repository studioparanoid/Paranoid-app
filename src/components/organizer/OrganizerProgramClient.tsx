...
const fieldClass = "min-h-11 w-full rounded border border-[var(--border-strong)] bg-[var(--surface)] px-3 text-sm text-[var(--foreground)] outline-none focus:border-red-700";
...
export function OrganizerProgramClient({ eventId }: { eventId: string }) {
  ...
  return <main className="mx-auto max-w-6xl px-5 py-8 pb-28 lg:px-8 lg:py-12">
    <header className="flex flex-wrap items-end justify-between gap-5 border-b border-[var(--border)] pb-6">
      <div><p className="text-xs font-black uppercase text-red-600">Programa e lineup</p><h1 className="mt-2 text-3xl font-black text-[var(--foreground)]">{event?.title || "Evento"}</h1><p className="mt-2 text-sm text-[var(--foreground-muted)]">Programa {completion}% completo · rascunho protegido neste dispositivo</p></div>
    ...
    {mode === "visual" && <section className="mt-7">
      ...
      {zones.length > 0 && <div className="mt-6 grid gap-3 overflow-x-auto" style={{ gridTemplateColumns: `repeat(${zones.length}, minmax(220px, 1fr))` }}>
        {zones.map((zone) => <div key={zone} className="min-h-28 border-t-2 border-red-800 bg-[var(--surface)] p-3" onDragOver={(e) => e.preventDefault()} onDrop={(e) => { const localId = e.dataTransfer.getData("text/plain"); if (localId) updateItem(localId, "zone", zone); }}><h2 className="text-sm font-black">{zone}</h2>{items.filter((item) => item.zone === zone).map((item) => <div key={item.localId} draggable onDragStart={(e) => e.dataTransfer.setData("text/plain", item.localId)} className="mt-3 cursor-grab border-l-2 border-[var(--border-strong)] pl-3 text-xs"><strong>{item.startTime}</strong><p className="truncate">{item.title || "Sem título"}</p></div>)}</div>)}
      </div>}
    ...
    {message && <p role="status" className="mt-5 border-l-2 border-red-700 pl-3 text-sm text-[var(--foreground-secondary)]">{message}</p>}
