  ...
  return <nav aria-label="Navegação principal" className="brand-surface fixed inset-x-0 bottom-0 z-50 border-t border-[var(--brand-border)] bg-[var(--header-background)] px-2 pb-[calc(0.45rem+env(safe-area-inset-bottom))] pt-2 backdrop-blur-lg lg:hidden">
    <div className="mx-auto grid max-w-lg grid-cols-5 gap-1">
      {mainNavigation.map((item) => {
        ...
        return <Link key={item.href} href={item.href} aria-current={active ? "page" : undefined} className={`interactive pressable focus-ring relative flex min-h-12 flex-col items-center justify-center gap-1 rounded px-1 py-1.5 text-center text-[10px] font-bold ${active ? "text-[var(--brand-foreground)]" : "text-[var(--brand-muted)] hover:text-[var(--brand-foreground)]"}`}>
