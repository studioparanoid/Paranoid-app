...
export function FilterDrawer({ open, title = "Filtros", onClose, children, footer }: { open: boolean; title?: string; onClose: () => void; children: React.ReactNode; footer?: React.ReactNode }) {
  ...
  return <div className="ui-overlay fixed inset-0 z-[90]" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <aside ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-label={title} className="ui-sheet absolute inset-x-0 bottom-0 flex max-h-[88dvh] flex-col rounded-t-lg border-t border-border bg-background text-foreground sm:inset-y-0 sm:left-auto sm:w-full sm:max-w-md sm:rounded-none sm:border-l sm:border-t-0">
      <div className="min-h-0 overflow-y-auto overscroll-contain p-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))] [-webkit-overflow-scrolling:touch] sm:p-7">
