...
export function FilterDrawer({ open, title = "Filtros", onClose, children, footer }: { open: boolean; title?: string; onClose: () => void; children: React.ReactNode; footer?: React.ReactNode }) {
  ...
  return <div className="ui-overlay fixed inset-0 z-[90]" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <aside ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-label={title} className="ui-sheet absolute inset-x-0 bottom-0 max-h-[88vh] overflow-y-auto rounded-t-lg border-t border-zinc-800 bg-[#0b0b0b] p-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))] text-[#f2f1ec] sm:inset-y-0 sm:left-auto sm:w-full sm:max-w-md sm:rounded-none sm:border-l sm:border-t-0 sm:p-7">
      <div className="sticky top-0 z-10 flex items-center justify-between bg-[#0b0b0b] pb-5"><h2 className="text-xl font-black">{title}</h2><IconButton label="Fechar filtros" variant="secondary" onClick={onClose}><AppIcon name="close" /></IconButton></div>
