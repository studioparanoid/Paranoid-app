...
export function UserTicketsClient() {
  ...
  return <div>
    ...
    <section key={tab} className="content-transition mt-6">{visible.length === 0 ? <EmptyState title="Ainda não tens bilhetes." actionLabel="Ver Agenda" actionHref="/agenda" /> : <div className="divide-y divide-border border-y border-border">{visible.map((ticket) => {
      const ticketStatus = status(ticket); const open = openTicketId === ticket.id;
      return <article key={ticket.id} className="card-hover py-5"><div className="flex items-start gap-4">{ticket.event?.image_url && <span className="relative h-20 w-20 shrink-0 overflow-hidden rounded"><Image src={ticket.event.image_url} alt="" fill sizes="80px" unoptimized className="object-cover" /></span>}<div className="min-w-0 flex-1"><h2 className="text-lg font-black">{ticket.event?.title || "Evento"}</h2><p className="mt-1 text-sm text-foreground-muted">{ticket.event?.display_date || "Data por definir"}{ticket.event?.display_time ? ` · ${ticket.event.display_time}` : ""}</p><p className="mt-1 text-xs text-foreground-muted">{[ticket.event?.venue_name, ticket.event?.city].filter(Boolean).join(" · ")}</p><p className="mt-2 text-xs font-bold text-foreground-secondary">{ticket.quantity} {ticket.quantity === 1 ? "bilhete" : "bilhetes"}</p></div><StatusBadge label={ticketStatus.label} tone={ticketStatus.tone} /></div>
      <Button type="button" onClick={() => setOpenTicketId(open ? "" : ticket.id)} aria-expanded={open} aria-controls={`ticket-${ticket.id}`} variant="secondary" className="mt-4">{open ? "Fechar bilhete" : "Ver bilhete"}</Button>
      {open && <div id={`ticket-${ticket.id}`} className="scale-in mt-5 rounded-md border border-border bg-background-subtle p-5"><p className="text-center text-xs font-bold uppercase tracking-[0.2em] text-foreground-muted">Código de entrada</p>{ticket.status === "reserved" && <div className="mt-4"><TicketQrCode value={ticket.check_in_code} /></div>}<p className="mt-4 text-center font-mono text-lg font-black">{ticket.check_in_code}</p><div className="mt-5 flex flex-wrap justify-center gap-2">{ticket.event && <Link href={`/eventos/${ticket.event.slug}`} className="pressable focus-ring rounded-full border border-border-strong px-4 py-2 text-xs font-bold text-foreground-secondary hover:text-foreground">Ver evento</Link>}{ticket.status === "reserved" && <Button variant="danger" size="sm" onClick={() => setCancelTicketId(ticket.id)}>Cancelar reserva</Button>}</div></div>}
      </article>;
    })}</div>}</section>
