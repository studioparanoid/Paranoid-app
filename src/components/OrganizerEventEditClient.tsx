  ...
  useEffect(() => {
    async function loadEvent() {
      ...
      const { data: eventData, error: eventError } = await supabase
        .from("events")
        .select(
          "id,slug,title,city,venue_id,venue_name,organizer_id,organizer_name,start_at,end_at,start_date,end_date,is_multi_day,display_date,display_time,category,price,description,image_url,featured,status"
        )
        .eq("id", eventId)
        .maybeSingle();
      ...
      if (!loadedEvent.organizer_id) {
        setMessage("Este evento não está ligado a um organizador.");
        setLoading(false);
        return;
      }
      ...
      const { data: membershipData } = await supabase
        ...
        .select("organizer_id")
        .eq("user_id", user.id)
        .eq("organizer_id", loadedEvent.organizer_id)
        .maybeSingle();
      ...
      setArtistsText(getArtistNamesFromRelationRows(relationRows).join(", "));
    ...
    return (
      <main className="min-h-screen bg-[#0b0b0b] px-5 py-8 pb-28 text-[#f2f1ec]">
        <section className="mx-auto max-w-md">
          ...
          <h1 className="text-4xl font-black">Evento não disponível.</h1>
    ...
    <main className="min-h-screen bg-[#0b0b0b] px-5 py-8 pb-28 text-[#f2f1ec]">
      <section className="mx-auto max-w-md">
        ...
        <p className="mt-5 text-base text-zinc-400">
          Edita os dados públicos do teu evento. Destaques e publicação ficam
          reservados à Paranoid.
        </p>
